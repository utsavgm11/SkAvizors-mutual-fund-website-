...
const DreamVacationCalculator = ({
  ...
  useEffect(() => {
    const newErrors = {};
    if (yearsUntilTrip < 0 || yearsUntilTrip > 40) newErrors.yearsUntilTrip = 'Value between 0-40';
    ...
    setYearsLeft(yearsUntilTrip);
    setExistingInvestment(savedAmount);

    const inflationFactor = 1 + inflationRate / 100;
    const future = currentCost * Math.pow(inflationFactor, yearsUntilTrip);
    setFutureCost(future);

    const monthlyReturnRate = rateOfReturn / 100 / 12;
    const totalMonths = yearsUntilTrip * 12;
    const presentValue = savedAmount * Math.pow(1 + rateOfReturn / 100, yearsUntilTrip);
    const futureValue = future;

    if (yearsUntilTrip === 0 || totalMonths === 0) {
      setMonthlySavings(0);
    } else {
  ...
  }, [yearsUntilTrip, currentCost, savedAmount, inflationRate, rateOfReturn]);
  ...
  if (inputMode === 'input') {
    return (
      <div className={`${WHITE_CARD} ${BORDER_RADIUS} shadow p-6 space-y-8`}>
        {[
          {
            label: 'After how many years do you wish to go on your holiday?',
            name: 'yearsUntilTrip',
            value: yearsUntilTrip,
            min: 0,
            max: 40,
            step: 1,
            error: errors.yearsUntilTrip,
            tooltip: 'Number of years until your holiday trip.',
          },
          {
            label: 'How much does this trip cost today?',
            name: 'currentCost',
            value: currentCost,
            min: 0,
            max: 100000000,
            ...
            tooltip: 'Current estimated cost of your holiday.',
          },
          {
            label: 'How much money can you set aside for your holiday at the moment?',
            name: 'savedAmount',
            value: savedAmount,
            min: 0,
            max: 100000000,
            ...
            tooltip: 'Current savings towards your holiday goal.',
          },
          {
            ...
            tooltip: 'Expected yearly inflation rate.',
          },
          {
            ...
            tooltip: 'Expected yearly return on investments.',
          },
