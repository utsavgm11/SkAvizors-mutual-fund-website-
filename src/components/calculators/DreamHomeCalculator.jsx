...
const DreamHomeCalculator = ({
  ...
  const [homeLoanFunding, setHomeLoanFunding] = useState(0);

  useEffect(() => {
    const newErrors = {};

    if (yearsUntilHome < 0 || yearsUntilHome > 40) newErrors.yearsUntilHome = 'Value between 0-40';
    if (currentCost < 0 || currentCost > 100000000) newErrors.currentCost = 'Value between 0-100,000,000';
    if (homeLoanPercent < 0 || homeLoanPercent > 100)
      newErrors.homeLoanPercent = 'Value between 0-100';
    ...
    if (Object.keys(newErrors).length > 0) {
      setFutureCost(0);
      setMonthlySavings(0);
      setHomeLoanFunding(0);
      return;
    }

    // Calculate future cost adjusted for inflation
    const inflationFactor = 1 + inflationRate / 100;
    const future = currentCost * Math.pow(inflationFactor, yearsUntilHome);
    setFutureCost(future);

    // Calculate loan amount based on percentage
    const loanAmount = (homeLoanPercent / 100) * future;
    setHomeLoanFunding(loanAmount);

    // Calculate amount to save (future cost minus loan)
    const amountToSave = future - loanAmount;

    // Calculate monthly savings needed to reach amountToSave
    const monthlyReturnRate = rateOfReturn / 100 / 12;
    const totalMonths = yearsUntilHome * 12;
    // Assuming current savedAmount grows at rateOfReturn
    const presentValue = savedAmount * Math.pow(1 + rateOfReturn / 100, yearsUntilHome);

    if (yearsUntilHome === 0 || totalMonths === 0) {
      setMonthlySavings(0);
    } else {
  ...
  }, [yearsUntilHome, currentCost, homeLoanPercent, savedAmount, inflationRate, rateOfReturn]);
  ...
  if (inputMode === 'input') {
    return (
      <div className={`${WHITE_CARD} ${BORDER_RADIUS} shadow p-6 space-y-8`}>
        {[
          {
            label: 'After how many years do you wish to own your dream home?',
            name: 'yearsUntilHome',
            value: yearsUntilHome,
            min: 0,
            max: 40,
            step: 1,
            error: errors.yearsUntilHome,
            tooltip: 'Number of years until you plan to own the house.',
          },
          {
            label: 'How much do you think your dream home costs today?',
            name: 'currentCost',
            value: currentCost,
            min: 0,
            max: 100000000,
            ...
            tooltip: 'Current estimated cost of your dream home.',
          },
          {
            label: 'What percentage of this cost would you like to go for a home loan?',
            name: 'homeLoanPercent',
            value: homeLoanPercent,
            min: 0,
            max: 100,
            step: 1,
            error: errors.homeLoanPercent,
            tooltip: 'Percentage of house cost to cover via loan.',
          },
          {
            label: 'How much money can you set aside for your dream home at the moment?',
            name: 'savedAmount',
            value: savedAmount,
            min: 0,
            max: 100000000,
            ...
            tooltip: 'Current savings towards your dream home.',
          },
          {
  ...
  return (
    <div className={`${ORANGE_BG} ${BORDER_RADIUS} shadow-inner p-6 text-blue-900 font-semibold space-y-4`}>
      ...
      <div className="grid grid-cols-2 gap-6 text-lg">
        ...
        <div>
          <span className="block text-blue-800 font-medium mb-1">Home Loan Funding</span>
          <span>₹ {homeLoanFunding.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
        </div>
